...
- Install MAMP
- Root: `/Applications/MAMP/htdocs/kards-v1`
- Global NPM package requirements
	- `npm i -g bower@1.8.8`
	- `npm i -g handlebars@3.0.3`
	- `npm i -g grunt-cli@1.2.0`
	- `npm i -g ruby`
	- `gem install sass` (may need sudo)
	- `npm install, bower install`
- Crunch handlebars templates `handlebars --amd app/templates/*.hbs -f app/compiled-templates.js`
- Run grunt to compress and watch for SCSS changes `$ grunt`
- May need to install missing jointjs `bower install jointjs`
- Apply polyfill to `/app/main.js` due to chromium updates
...
- There is provision from the original boilerplate for testing frameworks and other task runners but a lot of this is not hooked up or used.
...
An app for extracting medical algorithms from the minds of medical experts  into a question and answer flow that provides individualised information.
...
and 
- ‘none of the above’ which is reserved for future use for ‘multiple choice’ types of questions
